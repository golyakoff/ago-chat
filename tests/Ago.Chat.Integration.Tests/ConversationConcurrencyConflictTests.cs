...
public class ConversationConcurrencyConflictTests(PostgresFixture fixture)
{
    ...
    public async Task AssignConversation_RacedOnceByAConcurrentMessageSend_RetriesAndSucceeds()
    {
        ...
        var handler = new AssignConversationHandler(
            racingRepository, new ConversationAssignmentLog(db), new PermissionChecker(db), new OperatorRepository(db),
            new OperatorRoleRepository(db), new OperatorCapacityStore(db), new EfUnitOfWork(db), new UuidV7Generator(), new SystemClock());
