...
public sealed class OperatorInviteSeatLimitConcurrencyTests(ConcurrencyTestFixture fixture, ITestOutputHelper output)
{
    ...
    public async Task ConcurrentRedemptions_RacingForTheOneRemainingSeat_ExactlyOneSucceeds()
    {
        ...
        // seat-limit race) must not be pre-empted by an unrelated EmailMismatch on every attempt.
    ...
    public async Task ARejectedInvite_IsStillRedeemableAfterASeatOpensUp()
    {
        ...
        var rejected = await RedeemAsync(invite.CodeHash, invite.Email, "redeemer-rejected");
        ...
        var succeeded = await RedeemAsync(invite.CodeHash, invite.Email, "redeemer-rejected");
    ...
    private async Task<List<GeneratedInvite>> GenerateInvitesAsync(Seed seed, int count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            var invite = OperatorInvite.Generate(
                new OperatorInviteId(Guid.NewGuid()), seed.SiteId, seed.RoleId, codeHash, email,
