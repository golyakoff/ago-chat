...
/// attributes and endpoint metadata, which is exactly what the named-policy registry is for. What is
/// here is applied to a route or a route group by delegate instead, so a name would buy nothing.
///
/// <para>`12-05`: this class briefly held a second method, `NotThePlatformOwner`, which `12-04` put
/// on `POST /api/v1/sites`. It is gone - `SitesEndpoints` and `adr/0063` carry why - and this is now
/// a one-method class again. Kept as a class rather than folded back into
/// <c>AttachmentEndpoints</c>: the reason `17-06` moved it out (a policy a test must be able to
/// exercise) is unchanged by there being one of them.</para>
/// </summary>
