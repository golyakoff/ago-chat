    ...
    IClock clock)
{
    ...
    private static readonly string[] OperatorRolePermissions =
        [
            ...
            // `23-78`: ConversationAttachmentUploadGrant joins the Operator set - see Permission's own
            // remarks on why this, unlike ConversationBlock, is Operator- rather than Admin-scoped.
    ...
    private static readonly string[] AdminRolePermissions =
        [
            ...
            // `24-10`: ConversationBlock joins the Admin set - the same granular, compliance-shaped
            // placement ConversationErase/ConversationExport already have (Permission.ConversationBlock's
            // own remarks on why it is its own permission rather than a reuse).
